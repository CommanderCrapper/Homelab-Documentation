Summary
-------------
Started in 2025, this repository is dedicated to the homelab I run at home, used for personal projects & learning.

Currently running Proxmox VE with multiple services that I use daily, as well as other friends of mine close to me.

Updates to the documentation will come rather infrequently, due to life & college.

The homelab & it's documentation are still very much a work in progress, as there are a lot of projects I have in mind that I will be working on in the future, listed below:

Future Goals
-------------
- Actually proper documentation (readthedocs.io most likely) with bug fixes I've encountered, examples of hosted projects, etc.
- ~~Port-forwarding (with firewall)~~
- Fully interactable network of devices I can interact with remotely under the subnet (privately hosted, not owned by a bigger corporation)
- 2FA for extra security measures, especially for if port-forwarding will be implemented
- Experimentation with AI natively once the homelab is able to handle LLMs (for learning how AIs work & pentesting)
- Upgraded hardware for the homelab
- Server rack with multiple servers & services running all at once
- Website hosting with my own online portfolio (like the one currently on my Github profile, although more professional & tidied up)
- Experimentation with network switches & other general-purpose server management devices
- Complete customization of the homelab's exterior (the case) with stickers & other various features
- Proper labeling of wires & hardware (individual wire tags, stickers on hard drives)

Credits
-------------
A majority of anything I've done couldn't have been without the assistance & encouragement of the friends & professors I've learned from, engaged with, & have come to know throughout the years of me taking an interest in computer science & cybersecurity. I am eternally grateful for all the opportunities I've been given & all the people I've come to know that have encouraged me to push myself forward.
